...
namespace equation {
	...
	*/
	namespace scheme {
		/**
		* Class representing Lax Wendroff schemes.It extends class ExplicitScheme
		* Implementing the virtual functions from class AbstractScheme and class ExplicitScheme
		* to get the numerical values with the particular expression of Lax Wendroff scheme.
		*/
		class Lax_Wendroff : public ExplicitScheme {
		public:
			/**
			* Constructor to create an instance of Lax Wendroff Scheme
			...
			*/
			explicit Lax_Wendroff(double(*Initial_Function)(double x, double u), double(*Analytical_Function)(double x, double t, double u),
				double LowerBoundary, double UpperBoundary, double Time_Level, int GridSize, double u, double CFL, int Left_BoundaryValue, int Right_BoundaryValue);
		private:
			/**
			* Override the pure virtual function, using the particular expression of Lax Wendroff scheme to get all points  
